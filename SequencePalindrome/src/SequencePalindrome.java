...
public final class SequencePalindrome {
    ...
    private static boolean isPalindrome(Sequence<Integer> s) {
        ...
        // TODO - fill in body //Palindrome means if DAD is revered it will still be DAD
        boolean result = true;
        int initial = 0;
        int last = s.length() - 1;
        while (last > initial) {
            Integer a = s.remove(initial); //remove the first and last letter and then compare it
            Integer b = s.remove(last - 1);
            if (!a.equals(b)) {
                result = false;
            }
            s.add(initial, a); //Restore the values
            s.add(last, b);
            initial++;
            last--;
        }
     ...
     * @ensures isPalindrome = (s = rev(s))
     */
    private static boolean isPalindromere(Sequence<Integer> s) {
        assert s != null : "Violation of: s is not null";
        ...
        if (s.length() == 0) {
            ...
            result = isPalindromere(s); //Assume the result is palindrome
            if (!front.equals(back)) { //Check if after switching it will be the same Eg: MOM
                result = false;
            }
            s.add(0, front); // restore the values to the original position
            s.add(s.length(), back);
        }
