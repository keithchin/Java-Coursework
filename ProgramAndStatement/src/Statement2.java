 ...
 * @author Levon Nie,Zou yupeng,Chua, Kee Sern
...
public class Statement2 extends StatementSecondary {
    ...
    private void createNewRep() {
        ...
        Sequence<Tree<StatementLabel>> NewSequence = this.rep
                .newSequenceOfTree();
        StatementLabel Root = new StatementLabel(Kind.BLOCK);
        this.rep.assemble(Root, NewSequence);
    ...
    public final void addToBlock(int pos, Statement s) {
        ...
        StatementLabel NewLabel = this.rep.disassemble(child);
        child.add(pos, NewStatement.rep);
        this.rep.assemble(NewLabel, child);
        s.clear();
    ...
    public final Statement removeFromBlock(int pos) {
         ...
         * code.
         */
        Statement2 NewStatement = this.newInstance();
        Sequence<Tree<StatementLabel>> child = this.rep.newSequenceOfTree();
        StatementLabel NewLabel = this.rep.disassemble(child);
        Tree<StatementLabel> TreeChild = child.remove(pos);
        NewStatement.rep.transferFrom(TreeChild);
        this.rep.assemble(NewLabel, child);
    ...
    public final void assembleIfElse(Condition c, Statement s1, Statement s2) {
        ...
        Sequence<Tree<StatementLabel>> children = this.rep.newSequenceOfTree();
        children.add(0, S1.rep);
        children.add(1, S2.rep);
        this.rep.assemble(label, children);
        S1.createNewRep();
        S2.createNewRep();
    ...
    public final Condition disassembleIfElse(Statement s1, Statement s2) {
        ...
        S1.rep = child.remove(0);
        S2.rep = child.remove(0);
        this.createNewRep();
    ...
    public final Condition disassembleWhile(Statement s) {
        ...
        Sequence<Tree<StatementLabel>> child = this.rep.newSequenceOfTree();
        StatementLabel NewLabel = this.rep.disassemble(child);
        NewStatement.rep = child.remove(0);
        this.createNewRep();

        return NewLabel.condition;
    }
