...
public class List2<T> extends ListSecondary<T> {
    ...
    @Override
    public void retreat() {
        assert this.leftLength() > 0 : "Violation of: this.right /= <>";
        if (this.leftLength() == 1) {
            this.lastLeft = this.preFront;
        } else if (this.rightLength() == 0) {
            this.moveToStart();
            while (this.rightLength() > 1) {
                this.lastLeft = this.lastLeft.next;
            }
        } else {
            Node nextPosition = this.lastLeft.next;
            T data = this.lastLeft.data;
            this.moveToStart();
            while (!nextPosition.data.equals(data)) {
                this.lastLeft = nextPosition;
                nextPosition = this.lastLeft.next;
            }
        }
    }
