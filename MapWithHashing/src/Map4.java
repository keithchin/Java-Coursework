...
public class Map4<K, V> extends MapSecondary<K, V> {
    ...
    public final void add(K key, V value) {
        ...
        this.hashTable.entry(location).add(key, value);
        this.size++;
    ...
    public final Pair<K, V> remove(K key) {
        ...
        int location = mod(key.hashCode(), this.hashTable.length());
        Map.Pair<K, V> temp = this.hashTable.entry(location).remove(key);
        this.size--;
    ...
    public final Pair<K, V> removeAny() {
        ...
        Map.Pair<K, V> temp = null;

        for (int i = 0; i < this.hashTable.length(); i++) {
            if (this.hashTable.entry(i).size() > 0) {
                temp = this.hashTable.entry(i).removeAny();
                this.size--;
                break;
            }
        }
    ...
    public final V value(K key) {
        ...
        int location = mod(key.hashCode(), this.hashTable.length());
        V temp = this.hashTable.entry(location).value(key);
    ...
    public final boolean hasKey(K key) {
        ...
        int location = mod(key.hashCode(), this.hashTable.length());
        boolean ans = this.hashTable.entry(location).hasKey(key);
