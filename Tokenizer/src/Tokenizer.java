...
public final class Tokenizer {
    ...
    private static String nextWordOrSeparator(String text, int position) {
        ...
        boolean isNotSep = SEPARATORS.indexOf(text.charAt(position)) < 0;
        //        if (SEPARATORS.indexOf(a) > 0) {
        //            while (SEPARATORS.indexOf(a) > 0 && position < text.length() ) {
        //                next = next + a;
        //                position++;
        //                if (position < text.length()) {
        //                    a = text.charAt(position);
        //                }
        //            }
        //        } else {
        while (position < text.length()
                && SEPARATORS.indexOf(a) < 0 == isNotSep) {
            next = next + a;
            position++;
            if (position < text.length()) {
                a = text.charAt(position);
    ...
    public static Queue<String> tokens(SimpleReader in) {
        ...
        while (!in.atEOS()) {
            ...
            while (count < a.length() - 1) {
                String word = nextWordOrSeparator(a, count);
                count += word.length() + 1;
