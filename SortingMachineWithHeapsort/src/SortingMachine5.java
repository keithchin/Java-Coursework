...
public class SortingMachine5<T> extends SortingMachineSecondary<T> {
    ...
    private static <T> void siftDown(Array<T> array, int top, int last,
        ...
        assert isHeap(array, 2 * top + 2, last, order) : ""
                ...
                + " [relation computed by order.compare method])";
                /*
                 * Impractical to check last requires clause; no need to check
                 * the other requires clause, because it must be true when using
                 * the Array representation for a complete binary tree.
                 */
        ...
        if (leftPos <= last && rightPos <= last) {
            left = array.entry(leftPos);
            right = array.entry(rightPos);
            // if left is smaller
